...
Scalar to_scalar(std::basic_string_view<uint8_t> be) noexcept {
    mpz_t m;
    mpz_init(m);
...
static std::optional<libff::alt_bn128_Fq2> decode_fp2_element(std::basic_string_view<uint8_t> bytes64_be) noexcept {
    assert(bytes64_be.size() == 64);
